...
ERRORS = {
    "prof":     "😴 LM Studio не запущен! Запусти сервер и попробуй снова, троечник!",
    "survival": "🏃 Профессор сбежал! Запусти LM Studio и попробуй снова!",
    None:       "❌ Запусти LM Studio → Local Server → Start Server!"
}
...
async def generate_reply(message: types.Message, uid: int, user_text: str):
    ...
    try:
        await message.answer("⏳ Думаю... подожди немного!")
        ...
        if "choices" not in data or not data["choices"]:
            await message.answer("⏳ Модель не ответила. Попробуй ещё раз!")
            return
...
async def main():
    print("🤖 Бот ВЫЖИВШИЙ СТУДЕНТ (LM Studio) запущен!")
